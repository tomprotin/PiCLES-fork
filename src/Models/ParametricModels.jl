...
export Parametric2D, init_particles!, init_StateArray, mark_boundary, reset_boundary!, fields
export fields
...
"""
    Parametric2D{Grid, Lay, Tim, Clo, stat, PC, Ovar, Osys, Oses, Odev, bl_flag, bl, wnds, cur}

    Parametric2D is a model for simulating wave growth in a 1D domain. 
...
"""
mutable struct Parametric2D{Grid<:AbstractGrid,
                        Lay,
                        Tim,
                        Clo,
                        Int,
...
"""  
Parametric2D(; grid, winds, ODEsys, ODEvars, layers, clock, ODEsets, ODEdefaults, currents, periodic_boundary, CBsets)
This is the constructor for the Parametric2D model. The inputs are:
...
Return a flattened `NamedTuple` of the State vector for a `Parametric2D` model.
"""
...
function Base.show(io::IO, ow::Parametric2D)

    sys_print = ow.ODEsystem
    print(io, "Parametric2D ", "\n",
        "├── grid: ", ow.grid, "\n",
        "├── layers: ", ow.layers, "\n",
        "├── clock: ", ow.clock,
        "├── State: ", size(ow.State), "\n",
